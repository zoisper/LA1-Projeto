...
COORDENADA jog_minimax (ESTADO e, int profundidade)
{
    ...
    if (obter_jogador_atual(e) == 1)
        index = best_index_palyer_1(scores, len, lista);
    else
        index = best_index_palyer_2(scores, len, lista);
...
int best_index_palyer_2 ( int array[],  int size, LISTA jogadas_possivies )
{
    int i, r;
...
int best_index_palyer_1 ( int array[],  int size, LISTA jogadas_possivies  )
{
    int i, r;
    COORDENADA coord = obter_coordenada_lista(jogadas_possivies, 0);
    r = 0;
